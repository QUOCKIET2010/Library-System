...
def page_home(lib):
    ...
    for i, book in enumerate(paginated_books):
        with cols[i % 4]:
            with st.container():
                st.markdown(f"""<div class="modern-card"><img src="{book.image}" style="width:100%; height:180px; object-fit:cover;"><div style="padding:12px;"><div style="font-weight:bold; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{book.title}</div><div style="font-size:0.85rem; color:#666;">{book.author}</div><div style="font-size:0.75rem; color:#999; margin-top:4px;">ID: {book.id}</div></div></div>""", unsafe_allow_html=True)
                b1, b2 = st.columns(2)
    ...
    with tab1:
        ...
        for s in active:
            ...
            with st.container(border=True):
                ...
                if is_late: date_html += f"<div style='color:#dc2626; font-size:0.85em; font-weight:bold; margin-top:4px;'>⚠️ Quá hạn! Phạt: {s.get_estimated_fine():,}đ</div>"
...
def page_admin_system(lib):
    ...
    with t1:
        ...
        m3.metric("Đang Mượn", active_loans)
        st.divider()
        ...
        for s in lib.slips:
            for item in s.items:
                ...
                if bk: cat_stats[bk.category] = cat_stats.get(bk.category, 0) + 1
